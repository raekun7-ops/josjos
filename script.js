...
function showUnitPicker() {
  ...
  const btnStyle = `
    width:100%;min-height:72px;padding:0 24px;
    border:0;border-radius:10px;cursor:pointer;
    color:white;background:linear-gradient(180deg,#052699,#001976);
    font-family:inherit;font-size:1.9rem;font-weight:950;text-transform:uppercase;
    box-shadow:0 10px 30px rgba(0,25,118,0.22);
    transition:transform 0.16s,box-shadow 0.16s;
  `;

  overlay.innerHTML = `
    <div style="text-align:center;padding:40px 24px;max-width:520px;width:100%;">
      <p style="margin:0 0 6px;color:#5570b8;font-size:0.8rem;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;">Pilih Unit</p>
      <h1 style="margin:0 0 40px;color:#001976;font-size:clamp(2.4rem,8vw,5rem);font-weight:950;line-height:0.95;text-transform:uppercase;">Laporan Harian</h1>
  ...
  function pick(unitKey) {
    const passwords = getUnitPasswords();
...
function showAdminPasswordEditor(unitList) {
  const passwords = getUnitPasswords();
  const modal = document.createElement("div");
  modal.style.cssText = `
    position:fixed;inset:0;z-index:10003;
  ...
  modal.innerHTML = `
    <div style="background:white;border-radius:18px;padding:40px 32px;max-width:500px;width:100%;
      ...
      <div>${rows}</div>
      <div id="adminSaveMsg" style="text-align:center;color:#16a34a;font-weight:800;font-size:0.88rem;min-height:20px;margin-top:12px;"></div>
      <div style="display:flex;gap:12px;margin-top:20px;">
  ...
  modal.querySelector("#adminSave").addEventListener("click", () => {
    const newPasswords = {};
    modal.querySelectorAll("[data-unit-pw]").forEach(inp => {
      newPasswords[inp.dataset.unitPw] = inp.value.trim().toUpperCase();
    });
    saveUnitPasswords(newPasswords);
    const msg = modal.querySelector("#adminSaveMsg");
    msg.textContent = "✓ Password berhasil disimpan!";
...
// Konfigurasi Firebase — ganti dengan project Firebase Anda sendiri
...
function cloudSave(data) {
  if (!FIREBASE_CONFIG.dbUrl || !currentUnit) return;
  // Debounce: tunggu 800ms setelah stop mengetik
  clearTimeout(_cloudSaveTimer);
  _cloudSaveTimer = setTimeout(async () => {
    setSyncStatus("⟳ Menyimpan...", "#f7b731");
    try {
      const url = `${FIREBASE_CONFIG.dbUrl}/units/${currentUnit}.json`;
      ...
      setSyncStatus("✓ Tersimpan", "#16a34a");
      setTimeout(() => setSyncStatus("", ""), 2500);
    } catch (err) {
      setSyncStatus("✗ Gagal sync", "#c72f54");
...
async function cloudLoad(starterData) {
  if (!FIREBASE_CONFIG.dbUrl || !currentUnit) return null;
  setSyncStatus("⟳ Memuat data...", "#f7b731");
  try {
    const url = `${FIREBASE_CONFIG.dbUrl}/units/${currentUnit}.json`;
    const res = await fetch(url);
    if (!res.ok) throw new Error("HTTP " + res.status);
    ...
    if (data && typeof data === "object") {
      setSyncStatus("✓ Data dimuat", "#16a34a");
    ...
    setSyncStatus("✗ Offline — data lokal", "#c72f54");
...
function render() {
  ...
  // Auto-scroll ke tengah (mobile)
  setTimeout(() => {
    const maxX = document.body.scrollWidth - window.innerWidth;
    if (maxX > 60) window.scrollTo({ left: maxX / 2, top: 0, behavior: 'instant' });
  }, 100);
}
...
function initApp() {
  ...
  // Load dari localStorage dulu (tampil cepat)
  state = loadState();
  unitTitle.textContent = state.title || getUnitConfig().starterData.title;
  render();

  // Lalu coba cloud (replace jika ada data lebih baru)
...
function getExportColumns(plans) {
  ...
  const mantriWidth = Math.max(185, ...state.mantri.map((mantri) => textWidth(mantri.name, "900 30px Arial") + 44));
...
(function autoScrollToCenter() {
  function doScroll() {
    // Scroll horizontal ke tengah halaman agar tabel terlihat tengah
    const maxX = document.body.scrollWidth - window.innerWidth;
    if (maxX > 0) {
  ...
  if (document.readyState === 'complete') {
    ...
    window.addEventListener('load', () => setTimeout(doScroll, 300));
  }
})();
